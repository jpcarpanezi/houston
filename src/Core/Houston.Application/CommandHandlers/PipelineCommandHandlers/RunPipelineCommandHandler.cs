...
namespace Houston.Application.CommandHandlers.PipelineCommandHandlers {
	public class RunPipelineCommandHandler : IRequestHandler<RunPipelineCommand, Core.Commands.ResultCommand> {
		...
		public async Task<Core.Commands.ResultCommand> Handle(RunPipelineCommand request, CancellationToken cancellationToken) {
			var pipeline = await _unitOfWork.PipelineRepository.GetActive(request.Id);
			if (pipeline is null) {
				return new Core.Commands.ResultCommand(HttpStatusCode.NotFound, "The requested connector could not be found.", "connectorNotFound");
			}
			...
			try {
				...
				return new Core.Commands.ResultCommand(HttpStatusCode.InternalServerError, "Error while trying to run the pipeline.", "cannotRunPipeline");
			}
